...
const glitchTargets = document.querySelectorAll('[data-contact-glitch]');
...
const setupGlitchText = (target) => {
  const isDetail = target.classList.contains('contact-glitch-text--detail');
  const profile = isDetail
    ? {
        tail: 0.045,
        maxEnergy: 0.78,
        restEnergy: 0.5,
  ...
  const trailClip = (head, lag, width) => {
...
if (glitchTargets.length && finePointer.matches && !reduceMotion.matches) {
  glitchTargets.forEach(setupGlitchText);
}
