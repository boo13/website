import './styles.css';
import { gsap, ScrollTrigger, initSmooth, getHeroEls, primeGPU, prefersReducedMotion } from './shared.js';

const PRESETS = {
  restrained: { zoomScale: 1.15, blurMax: 8, scrollDistance: '150%', scrubAmount: 1.5 },
  aggressive: { zoomScale: 2.5, blurMax: 20, scrollDistance: '200%', scrubAmount: 1.2 },
  zdepth:     { zoomScale: 1.25, blurMax: 18, scrollDistance: '300%', scrubAmount: 4.0 },
};
...
function applyPreset(name) {
  ...
  document.getElementById('tw-scrub-val').textContent = p.scrubAmount.toFixed(1);
}
...
function syncLabels() {
  ...
  scale.addEventListener('input', () => {
    document.getElementById('tw-scale-val').textContent = parseFloat(scale.value).toFixed(2);
  });
  blur.addEventListener('input', () => {
    document.getElementById('tw-blur-val').textContent = blur.value + 'px';
  });
  scrub.addEventListener('input', () => {
    document.getElementById('tw-scrub-val').textContent = parseFloat(scrub.value).toFixed(1);
  });
}
